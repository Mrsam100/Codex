...
const InfoPage: React.FC<InfoPageProps> = ({ view, onBack }) => {
  const content = {
    about: {
      title: "About Codex",
      icon: <Info className="text-[#C9A962]" />,
      body: (
    ...
    terms: {
      ...
      icon: <ScrollText className="text-[#C9A962]" />,
      body: (
        <div className="space-y-6">
    ...
    faq: {
      ...
      icon: <HelpCircle className="text-[#C9A962]" />,
      body: (
        <div className="space-y-8">
          <div>
            <h4 className="text-[#F5F2E8] font-medium mb-2">How are connections made?</h4>
          ...
          <div>
            <h4 className="text-[#F5F2E8] font-medium mb-2">Can I use this offline?</h4>
          ...
          <div>
            <h4 className="text-[#F5F2E8] font-medium mb-2">How many fragments can I add?</h4>
    ...
    guide: {
        title: "How It Works",
        icon: <Book className="text-[#C9A962]" />,
        body: (
          <div className="space-y-6">
            <p>1. <strong>Capture:</strong> Use the input bar at the bottom to record a single thought, quote, or observation.</p>
            <p>2. <strong>Connect:</strong> As you add more, the AI will weave lines between them based on thematic similarity.</p>
            <p>3. <strong>Synthesize:</strong> Press <kbd className="bg-[#2A2A2C] px-1 rounded">⌘K</kbd> to open the Command Palette. Type a question like "What are my thoughts on simplicity?" and the "Librarian" will synthesize an answer from your combined fragments.</p>
            <p>4. <strong>Explore:</strong> Click and drag the void to move around. Use scroll or the plus/minus buttons to zoom.</p>
          </div>
        )
      }
  };
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-[#0A0A0B] overflow-y-auto">
      <div className="max-w-3xl mx-auto px-6 py-20">
        <button 
          onClick={onBack}
          className="flex items-center gap-2 text-[#8B7355] hover:text-[#F5F2E8] transition-colors mb-12 font-mono text-xs uppercase tracking-widest"
        >
        ...
        <div className="flex items-center gap-4 mb-8">
          {activeContent?.icon}
          <h1 className="font-cormorant text-4xl text-[#F5F2E8] tracking-tight">{activeContent?.title}</h1>
        </div>

        <div className="font-spectral text-lg leading-relaxed text-[#8B7355] border-t border-[#2A2A2C] pt-12">
          {activeContent?.body}
        </div>
      </div>
    </div>
